...
with st.expander("📘 What is linear regression? And why is it different?", expanded=True):
    st.markdown("""
    **Linearity** refers to the ability of an analytical method to produce results that are directly proportional to the concentration of the analyte in the sample. This assumption underpins many calibration procedures used in clinical and laboratory settings.

    A **linearity experiment** typically involves preparing a series of dilutions from a specimen and measuring them across five or more concentration levels. We then assess whether the measured values increase proportionally with concentration — in other words, whether the data form a straight line.

    **Why linear regression?**  
    Linear regression is used to **model and quantify** the relationship between an instrument's response (`Y`) and the known analyte concentration (`X`). It's preferred in method validation because it is:
    
    - **Simple** and interpretable
    - **Statistically efficient** when the linearity assumption is met
    - **Sufficient** in most analytical systems where the instrument behaves predictably
    - **Invertible** for calibration: once fitted, the equation can be used to estimate unknown concentrations from new measurements.

    **Calibration Curve**:  
    A calibration curve based on linear regression fits a model of the form:
    """)
    st.latex(r''' Y = \alpha + \beta x + \epsilon ''')
    st.markdown("This can be rearranged to solve for the unknown concentration from a measured response:")
    st.latex(r'''\bar{X} = \frac{Y - \hat{\alpha}}{\hat{\beta}}''')
    st.markdown("""
    This approach assumes that each reference standard has at least one reliable measurement. Importantly, **linearity ensures that the error structure remains consistent** across the range, enabling predictable performance.

    While more complex regression models (e.g., polynomial, logistic, or spline models) can capture nonlinear behavior, they are **not necessary unless there's evidence of curvature** in the calibration data. Linear regression remains the **default and preferred approach** unless nonlinearity significantly impacts accuracy or precision.

    In summary, linear regression provides a **transparent, reproducible, and statistically robust** method for assessing and applying analytical linearity. It is a cornerstone of method validation and instrument calibration.
    """)


with st.expander("🔍 Investigating Linearity Issues", expanded=False):   
    st.markdown("""
